...
| `.github/workflows/deploy.yml` | Публикация GitHub Pages из `main`  | При изменении CI/CD или production deploy   |
...
| Проверить открытые технические риски  | `docs/ASTRO_CODE_AUDIT.md`         |
...
до-Astro прототипу и brainstorming удалены из активного дерева. Текущий
`docs/ASTRO_CODE_AUDIT.md` остаётся рабочим реестром незакрытых технических
рисков; завершённую историю исправлений хранит Git. При редкой исторической
необходимости использовать Git, а не восстанавливать удалённые документы как
...
Рабочие изменения выполняются в ветках `codex/<short-task-name>`, созданных от
актуального `origin/main`. Коммиты и push идут только в такую ветку; merge или
push в `main` выполняются только по отдельной явной команде.
