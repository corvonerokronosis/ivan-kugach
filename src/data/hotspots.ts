...
const hotspots = validateHotspots([
  {
    ...
    text: joinText(
      "В этой версии прототипа вывеска работает как тестовая точка внимания.",
      "Здесь позже можно рассказать, как текст на фасаде вводит зрителя в сюжет,",
      "связывает бытовую сцену с конкретным местом и помогает считывать эпоху.",
    ),
  },
  {
    id: "crack",
    ...
    text: joinText(
      "Эта зона может стать поводом для короткой музейной ремарки о материальности старого дома.",
      "Пока здесь заглушка: в финальной версии можно говорить о фактуре штукатурки,",
      "следах старения и том, как художник работает с ощущением прожитого пространства.",
    ),
  },
  {
    id: "window",
    ...
    text: joinText(
      "Оконный проём удобно использовать как главный смысловой фокус.",
      "Внутри этой карточки потом можно разместить текст о композиции, ритме наличников",
      "и о том, как тёмное стекло собирает вокруг себя соседние детали.",
    ),
  },
  {
    id: "brick",
    ...
    text: joinText(
      "Нижний фрагмент с кирпичом хорошо подходит для разговора о слоях поверхности и ручной фактуре письма.",
      "Сейчас это временный текст-заполнитель, который можно заменить на описание колорита,",
      "ритма кладки или состояния архитектурной среды.",
    ),
  },
] satisfies Hotspot[]);
